    ...
    <div class="create">
        ...
        <div class="flex items-center justify-between mb-6">
            <div><!-- Log on to besten.com for more projects -->
                <h2 class="text-white-700 uppercase font-bold">Attendance for class :- {{ $class->class_name }}</h2>
            </div>
        ...
        <div class="w-full mt-8 bg-white rounded">
            <div class="flex items-center justify-between px-6 py-6 pb-0">
                ...
                <h3 class="text-white-700 uppercase font-bold"> Date: {{ date('Y-m-d') }}</h3>
            </div>
                ...
                <form action="{{ route('teacher.attendance.store') }}" method="POST">
                    @foreach ($class->students as $student)
                        <div class="flex items-center justify-between border border-gray-200">
                            @csrf
                            <div class="w-4/12 text-sm text-left text-white-600 py-2 px-4 font-semibold">{{ $student->user->name }}</div>
                            <div class="w-3/12 text-sm text-left text-white-600 py-2 px-4 font-semibold">{{ $student->roll_number }}</div>
