...
@section('content')
    <div class="create">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-white-700 uppercase font-bold">Attendance Report</h2>
            </div>
        ...
        <div class="w-full mt-8 bg-white rounded">
            <form action="{{ route('attendance.index') }}" method="GET" class="md:flex md:items-center md:justify-between px-6 py-6 pb-0">
                <div class="md:flex md:items-center mb-6 text-white-700 uppercase font-bold">
                    <div>
                        <label class="block text-white-500 font-bold md:text-right mb-1 md:mb-0 pr-4">
                            Report:
                        </label>
                    </div>
                    <div class="flex flex-row items-center bg-black-200 px-4 py-3 rounded">
                        <label class="block text-white-600 font-bold">
                            <input name="type" class="mr-2 leading-tight" type="radio" value="class" checked>
                            <span class="text-sm">Class</span>
                        </label>
                        <!-- <label class="ml-4 block text-white-600 font-bold">
                ...
                <div class="md:flex md:items-center mb-6 text-white-700 uppercase font-bold">
                    ...
                    <div class="block text-white-600 font-bold">
                        <div class="relative">
                            <select name="month" class="block font-bold appearance-none w-full bg-black-200 border border-gray-200 text-white-600 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
            ...
            <div class="w-full px-6 py-6">
                @foreach ($attendances as $classid => $datevalues)
                    <h2 class="bg-black-600 text-white font-semibold uppercase px-4 py-3">
                        class {{ $classid }}
                    </h2>
                    <div class="flex flex-col bg-black-200 mb-6">
                        @foreach ($datevalues as $key => $attendancevals)
                            <div class="text-left text-white-800 py-2 px-4 font-semibold">
                                <b><span >{{ $key }}</span></b>
                                <div class="flex flex-col justify-between bg-black-100">
                                    @foreach ($attendancevals as $vals => $attendance)
                                        <div class="flex flex-row justify-between w-64">
                                            <div class="text-sm text-left text-white-600 py-2 px-4 font-semibold">{{ $attendance->student->user->name }}</div>
                                            <div class="text-sm text-left text-white-600 py-2 px-4 font-semibold">
