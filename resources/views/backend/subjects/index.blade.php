...
@section('content')
    <div class="roles-permissions">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-white-700 uppercase font-bold">Subjects</h2>
            </div>
        ...
        <div class="mt-8 bg-white rounded border-b-4 border-gray-300">
            ...
            @foreach ($subjects as $subject)
                <div class="flex flex-wrap items-center text-white-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    <div class="w-3/12 px-4 py-3 text-sm font-semibold text-white-600 tracking-tight">{{ $subject->name }}</div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-white-600 tracking-tight">{{ $subject->subject_code }}</div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-white-600 tracking-tight">{{ $subject->teacher->user->name }}</div>
                    <div class="w-3/12 px-4 py-3 text-sm text-white-600 tracking-tight">{{ $subject->description }}</div>
