...
@section('content')
    <div class="roles-permissions">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-white-700 uppercase font-bold">Assign Role</h2>
            </div>
        ...
        <div class="mt-8 bg-white rounded border-b-4 border-gray-300">
            ...
            @foreach ($users as $user)
                <div class="flex flex-wrap items-center text-white-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    <div class="w-3/12 px-4 py-3 text-sm font-semibold text-white-600 tracking-tight">{{ $user->name }}</div>
                    <div class="w-5/12 px-4 py-3 text-sm font-semibold text-white-600 tracking-tight">{{ $user->email }}</div>
                    <div class="w-2/12 px-4 py-3 flex flex-wrap">
                        @foreach ($user->roles as $role)
                            <span class="bg-black-200 text-xs font-semibold text-white-600 tracking-tight px-3 py-1 border rounded-full">{{ $role->name }}</span>
                        @endforeach
                    </div>
