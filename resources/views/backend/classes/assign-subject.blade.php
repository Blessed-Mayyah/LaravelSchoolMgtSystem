...
@section('content')
    <div class="roles">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-white-700 uppercase font-bold">Assign Subject</h2>
            </div>
        ...
        <div class="w-full py-12">
            <h2 class="text-white-700 uppercase font-bold my-2">Students</h2>
            ...
            @foreach ($assigned->students as $student)
                <div class="flex items-center justify-between border border-gray-200 mb-px">
                    <div class="w-1/4 text-left text-white-600 py-2 px-4 font-medium">{{ $student->user->name }}</div>
                    <div class="w-1/4 text-left text-white-600 py-2 px-4 font-medium">{{ $student->user->email }}</div>
                    <div class="w-1/4 text-right text-white-600 py-2 px-4 font-medium">{{ $student->phone }}</div>
                    <div class="w-1/4 text-right text-white-600 py-2 px-4 font-medium">{{ $student->parent->user->name }}</div>
                </div>
            @endforeach
        </div>
