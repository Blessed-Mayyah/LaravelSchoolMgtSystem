...
@section('content')
    <div class="roles-permissions">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-white-700 uppercase font-bold">Parents</h2>
            </div>
        ...
        <div class="mt-8 bg-white rounded border-b-4 border-black">
            ...
            @foreach ($parents as $parent)
                <div class="flex flex-wrap items-center text-white-700 border-t-2 border-l-4 border-r-4 border-black-300">
                    <div class="w-3/12 px-4 py-3 text-sm font-semibold text-white-600 tracking-tight">{{ $parent->user->name }}</div>
                    <div class="w-3/12 px-4 py-3 text-sm font-semibold text-white-600 tracking-tight">{{ $parent->user->email }}</div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-white-600 tracking-tight">
                        @foreach ($parent->children as $children)
                            <span class="bg-black-200 text-xs font-normal px-2 py-px border rounded-full inline-flex my-px">
                                {{ $children->user->name }}
                            </span>
                        @endforeach
                    </div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-white-600 tracking-tight">{{ $parent->phone }}</div>
