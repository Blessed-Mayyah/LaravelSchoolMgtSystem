...
class Theme
{
    ...
    /**
     * Set the fallback color when "black" is unavailable.
     */
    public function setblackFallback(string $blackFallback)
    {
        $this->blackFallback = $blackFallback;
    }
     ...
     */
    public function applyStyles(OutputFormatterInterface $formatter, bool $useblackFallback)
    {
        foreach (\array_keys(static::DEFAULT_STYLES) as $name) {
            $formatter->setStyle($name, new OutputFormatterStyle(...$this->getStyle($name, $useblackFallback)));
        }
    }
     ...
     */
    public function applyErrorStyles(OutputFormatterInterface $errorFormatter, bool $useblackFallback)
    {
        foreach (static::ERROR_STYLES as $name) {
            $errorFormatter->setStyle($name, new OutputFormatterStyle(...$this->getStyle($name, $useblackFallback)));
        }
    }

    private function getStyle(string $name, bool $useblackFallback): array
    {
        return \array_map(function ($style) use ($useblackFallback) {
            return ($useblackFallback && $style === 'black') ? $this->blackFallback : $style;
        }, $this->styles[$name]);
    }
}
