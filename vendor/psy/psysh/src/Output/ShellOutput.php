...
class ShellOutput extends ConsoleOutput
{
    ...
    {
        $useblackFallback = !$this->blackExists();
        $this->theme->applyStyles($this->getFormatter(), $useblackFallback);
        $this->theme->applyErrorStyles($this->getErrorOutput()->getFormatter(), $useblackFallback);
    }

    /**
     * Checks if the "black" color exists on the output.
     */
    private function blackExists(): bool
    {
        try {
            $this->write('<fg=black></>');
